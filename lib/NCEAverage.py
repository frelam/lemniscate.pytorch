...
class NCEFunction(Function):
    ...
    def forward(self, x, y, memory, idx, params):
        ...
        indices_cpu = [i for i in range(int(K/2))]+[4096+i-int(K/2) for i in range(int(K/2))]
        ...
        indices3_cpu = [0] + [i for i in range(5, int(K+1))]
        indices3 = torch.cuda.LongTensor(indices3_cpu)
        if Z < 0:
            params[2] = out.index_select(1, indices3).mean() * outputSize  # Z = n/m*sum(exp(vfi/t))  n=outputSize
            Z = params[2].item()
